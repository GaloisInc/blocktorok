...
import           Language.Common.Units.SI          as SI
...
symbolTable = SI.siTable

asks :: Monad m => (SymbolTable -> a) -> Parser m a
asks f = pure (f symbolTable)


-- end reader monad hack
...
prefixUnitP =
  do prefixTab <- asks prefixTable
     ...
     results <- catMaybes <$> mapM (experiment . parseOne) assocs
...
unitStringP s =
  do  case MP.runParserT unitStringParser "" s of
       Left err -> err
       Right e -> case e of
