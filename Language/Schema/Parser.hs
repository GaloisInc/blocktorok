...
import           Language.Common                   (Located (..),
                                                    SourceRange (..))
import           Language.Common.Units.Parser      (parseUnit)
import           Language.Common.Units.SI          (siTable)
import           Language.Common.Units.SymbolTable (SymbolTable)
import           Language.Schema.Env               (Env, addRootType,
                                                    addTypeDef, emptyEnv,
                                                    lookupTypeDef)
...
parseSchema fp t =
    case result of
      ...
      (Right _, s)  -> Right s
  where
    result = State.runState (flip runReaderT siTable $ MP.runParserT (schema >> MP.eof) fp t) emptyEnv
